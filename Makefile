...
# Example: mysql://user:pass@tcp(127.0.0.1:3306)/recrutr-db?multiStatements=true&parseTime=true
ifdef MYSQL_DSN
  DB_URL := $(MYSQL_DSN)
else
...
.PHONY: migrate-up migrate-down migrate-version migrate-install env-print migrate-force dev run air-install
...
migrate-install:
	@echo "==> Installing golang-migrate with MySQL driver support..."
